...
# ---------------
# Bu Dosya MasiTa Tarafından kodlanmıştır
# Sizden proxy dosya adını isteyecek, örneğin: proxy.txt
# Ardından gönderi bağlantısı isteyecektir, örneğin: https://t.me/MasiTaOrj/1450
# Kaç tane Pencere istiyorsun [default:350]
# Not: Yalnızca [ HTTP / HTTPS ] Proxy'ler İle Çalışır !

# Kanalım : t.me/androidcraks
# WhatsApp : wa.me/+905414738616
# ---------------

system("title " + f"TeleView Coded by @MasiTaOrj")
print('''
████████╗███████╗██╗     ███████╗██╗   ██╗██╗███████╗██╗    ██╗
╚══██╔══╝██╔════╝██║     ██╔════╝██║   ██║██║██╔════╝██║    ██║
   ██║   █████╗  ██║     █████╗  ██║   ██║██║█████╗  ██║ █╗ ██║
...
''')
_proxy_file = input(' [MasiTaOrj] Proxy Dosyanızın İsmini Giriniz Default:[ proxy.txt ]: ')
link = input(' [MasiTaOrj] Telegram Post Bağlantısını Giriniz: ').strip().replace('https://', '').replace('http://', '')
_threads = int(input(' [MasiTaOrj] Kaç tane Pencere istiyorsun [ Default:200 ] :'))
...
def tit(): system("title " + f"TeleView Coded by @MasiTaOrj -- İstatistik: ({done}/{count_proxies}) -- Gönderilen: {sent} -- Kötü Proxy: {bad_proxy}")
...
def send_views():
    ...
    while True:
        ...
        try:
            ...
            print(' [✅✅] BASKIN GERCEKLESTİRİLDİ [✅✅]  ' + ' '+str(views_req.status_code))
            sent += 1
            done += 1
            tit()
